...
export default {
    ...
    methods: {
        ...
        play:function(){
            if(this.$refs.audio.paused){
                ...
                this.updateTime();  // 如果在播放就调用updateTime函数进行传值
            }else{
        ...
        // 因为当音乐播放时要获取当前时间，所以要给获取当前时间的函数，开启定时器
        updateTime: function(){
    ...
    watch:{
        playListIndex:function(){   // 监听，如果下标发生了改变就自动播放音乐
...
.FooterMusic .footerLeft img{
    width:1rem;
    height:1rem;
    border-radius:50%;
}
.FooterMusic .footerRight{
    width:20%;
    height:100%;
    display:flex;
    justify-content:space-between;
